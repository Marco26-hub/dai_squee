(() => {
  ...
  const labels = {arrival:"All'arrivo",card:'Carta adesso',bank:'Bonifico bancario'};
  ...
  let month = new Date(current.getFullYear(),current.getMonth(),1), occupied = null, quote = null;
  ...
  async function api(path,data,key) {
    if(location.protocol==='file:') throw new Error('Aprire il sito online per consultare disponibilità e prenotare.');
    const response=await fetch('/api/'+path,{method:data?'POST':'GET',headers:{'Content-Type':'application/json',...(key?{'Idempotency-Key':key}:{})},body:data?JSON.stringify(data):undefined,cache:'no-store',signal:AbortSignal.timeout(30000)});
    let result; try { result=await response.json(); } catch { throw new Error('Servizio momentaneamente non disponibile. Riprovate o contattate la struttura.'); }
    if(!response.ok) { const error=new Error(result.error||'Operazione non completata.');error.status=response.status;throw error; }
    return result;
  }
  ...
  function updateSelection() {
    ...
    if(el('continueBooking')) el('continueBooking').href='prenota.html?'+new URLSearchParams({apartment:roomSelect.value,checkin:arrival,checkout:departure});
    render();
  }
  function render() {
    el('calendarTitle').textContent=month.toLocaleDateString('it-IT',{month:'long',year:'numeric'});
    ...
    for(const name of ['Lun','Mar','Mer','Gio','Ven','Sab','Dom']) { const label=document.createElement('span');label.textContent=name;grid.append(label); }
    ...
    for(let i=1;i<=count;i++) {
      ...
      button.setAttribute('aria-label',new Date(date+'T12:00:00').toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'})+(checkoutBoundary?' disponibile come partenza':busy?' occupato':' disponibile'));
      button.setAttribute('aria-pressed',String(date===arrival||date===departure));
      button.addEventListener('click',()=>{
        if(arrival&&!departure&&date>arrival) {
          if(conflict(arrival,date)){status('Il periodo comprende notti occupate. Scegliete un altro intervallo.',true);return;}
          departure=date;
        } else {arrival=date;departure='';}
        invalidate();updateSelection();
        status(departure?'Periodo selezionato: '+arrival+' / '+departure:'Arrivo selezionato. Scegliete la data di partenza.');
      });grid.append(button);
    }
  }
  async function refresh() {
    ...
    try {
      ...
      if(!Array.isArray(data.unavailable)) throw new Error('Disponibilità non verificabile.');
      ...
      el('calendarStatus').textContent='Disponibilità aggiornata alle '+new Date().toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'})+'.';
      el('calendarStatus').classList.remove('error');
      if(arrival&&departure&&conflict(arrival,departure)) { invalidate();status('Il periodo selezionato non è più disponibile.',true); }
    } catch(error) {
      occupied=null;invalidate();el('calendarStatus').textContent=error.message+' Nessuna disponibilità presunta.';el('calendarStatus').classList.add('error');
  ...
  el('stayForm')?.addEventListener('submit',async event=>{
    ...
    if(!arrival||!departure||departure<=arrival||arrival<today) {status('Selezionate arrivo e partenza validi.',true);return;}
    const version=quoteVersion;el('quoteButton').disabled=true;status('Verifica del totale e della disponibilità…');
    try {
      const result=await api('quote',{apartment:roomSelect.value,checkin:arrival,checkout:departure,guests:Number(el('bookingGuests').value)});
      if(version!==quoteVersion)return;
      quote=result;el('quoteNights').textContent=result.quote.apartment+' · '+result.quote.nights+' notti';el('quoteAmount').textContent=money(result.quote.amount_cents);el('quoteTerms').textContent=result.quote.terms;
      el('bankTerms').textContent=result.quote.methods.includes('bank')?'Bonifico: '+result.quote.bank_instructions:'';
  ...
  function paymentLabel() {
    el('reserveButton').textContent=el('directBookingForm').elements.payment_method.value==='card'?'Prenota e paga con carta':'Conferma prenotazione con obbligo di pagamento';
  }
  function showResult(result) {
    ...
    title.textContent=result.status==='cancelled'?'Prenotazione annullata':result.method==='card'&&!result.paid?'Pagamento da completare':'Prenotazione confermata';box.append(title);
    add('Riferimento: '+result.reference);add(result.apartment+' · '+result.checkin+' / '+result.checkout);add('Totale: '+money(result.amount_cents)+' · '+labels[result.method]);
    if(result.paid)add('Pagamento registrato.');
    if(result.method==='bank'&&!result.paid&&result.status==='confirmed') { add(result.terms.bank_holder+' · IBAN '+result.terms.bank_iban);add('Causale: '+result.reference);add(result.terms.bank_instructions); }
    if(result.method==='arrival'&&!result.paid)add("Importo da corrispondere all'arrivo.");
    if(result.url&&result.method==='card') {const link=document.createElement('a');link.className='primary-link';link.href=result.url;link.textContent='Completa il pagamento';box.append(link);add('Le date restano riservate fino all’esito o alla scadenza della sessione di pagamento.');}
    add(result.terms.terms);
    if(result.email_error)add('Conferma email non inviata o non verificata. Conservate questo riferimento; la prenotazione è registrata.');
    const refreshButton=document.createElement('button');refreshButton.className='text-link';refreshButton.type='button';refreshButton.textContent='Aggiorna stato';
  ...
  el('directBookingForm')?.addEventListener('submit',async event=>{
    ...
    form.querySelectorAll('input').forEach(input=>input.disabled=true);el('stayForm').querySelectorAll('input,select').forEach(input=>input.disabled=true);render();status('Registrazione della prenotazione…');
    try {
      const result=await api('reserve',payload,requestKey);
      history.replaceState(null,'','prenota.html?receipt='+result.access_token);quote=null;requestKey=null;pendingPayload=null;showResult(result);status('');
      ...
      status(error.message+(pendingPayload?' Riprovate la stessa prenotazione per verificarne l’esito, senza duplicarla.':''),true);
    }
